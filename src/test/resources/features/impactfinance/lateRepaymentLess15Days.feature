@ImpactFinance
Feature: Late Repayment less than 15 days for Impact Finance

  Background: Log in as admin
    When I log in with username admin and password admin
    Then cloud should response with code 200

  Scenario: Late Repayment less than 15 days for Impact Finance
    ...
    When I actualize created loan on 2018-12-19
    Then cloud should response with code 200
    And message should be Actualize loan started

    When I make NORMAL_REPAYMENT for 108434.18 on 2018-12-19
    Then cloud should response with code 200
    And request should be created
    And not appear in Maker/Checker tab
...
"""
  ...
  "rows" : [ {
    ...
    "overdue" : "OVERDUE",
    "data" : [ 3, "2019-01-14", 96258.23, 12175.95, 2841.06, 0.00, 0.00, 108434.18, 811730.13 ],
    "status" : "UNPAID"
  }, {
    "overdue" : "OVERDUE",
  ...
  "totals" : [ null, null, 1000000.00, 84341.79, 31439.55, 188269.87, 28598.49, 867473.43, null ]
}
"""